...
def process_frames(api_key, parallelism=1):
    out_dir = "out"
    in_dir = "in"
        ...
        with active_jobs_lock:
            if active_jobs:
                ...
                for cancel_url in active_jobs.values():
                    ...
                    try:
                        requests.post(
                            cancel_url, headers={"Authorization": f"Bearer {api_key}"}
                        )
                    except:
    ...
    def process_image(i, image_path, total):
        ...
        payload = {
            "overrides": {
                ...
                "100": {"inputs": {"image": image_uri}},
                "104": {"inputs": {"value": random.randint(0, 2**32 - 1)}},
            }
        }
        ...
        try:
            while not shutting_down:
                response = requests.get(
                    result_url, headers={"Authorization": f"Bearer {api_key}"}
                )
